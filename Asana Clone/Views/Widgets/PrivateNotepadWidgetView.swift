        ...
        VStack(alignment: .leading) {
			...
			TextView(
				attributedText: Binding(
					get: {
						let newStr = try? NSKeyedUnarchiver.unarchivedObject(
							ofClass: NSAttributedString.self,
							from: asanaManager.currentMember?.notepad ?? .init()
						)
						
						print(newStr.debugDescription)

						return newStr ?? NSAttributedString(string: "")
					},
					set: { newValue in
						let data = try? NSKeyedArchiver.archivedData(withRootObject: newValue, requiringSecureCoding: true)

						asanaManager.currentMember?.notepad = data
					}
				),
